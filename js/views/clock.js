...
export default {
    render: () => {
        ...
        components.clocks.forEach(c => c.tags.forEach(tag => allTags.add(tag)));
        ...
        const tarjetasClocksHTML = components.clocks.map(clock => `
            <div class="component-card-layout filter-item" data-id="${clock.id}" data-tags="${clock.tags.join(',').toLowerCase()}">
                ...
                <div class="export-actions-panel">
                    <button class="action-copy-btn" data-type="html" data-code="${escapeAttribute(clock.htmlCode)}">
                        <span class="icon">&lt;/&gt;</span> Copiar HTML
                    </button>
                    <button class="action-copy-btn" data-type="css" data-code="${escapeAttribute(clock.cssCode)}">
                        <span class="icon">#</span> Copiar CSS
                    </button>
                    <button class="action-copy-btn" data-type="js" data-code="${escapeAttribute(clock.jsCode)}">
        ...
        return `
            <div class="components-container">
                ...
                <p class="subtitle">Relojes modernos, dinámicos e interactivos.</p>
                
                <div class="modern-filters-bar">
                    <div class="search-box">
                        <i class="fa-solid fa-magnifying-glass search-icon"></i>
                        <input type="text" id="component-search" placeholder="Buscar relojes...">
                    </div>
    ...
    init: () => {
        ...
        const fuse = new window.Fuse(components.clocks, {
            keys: ['name', 'tags'],
            threshold: 0.4,
            ignoreLocation: true
        });
