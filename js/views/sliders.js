...
export default {
    render: () => {
        ...
        components.sliders.forEach(s => s.tags.forEach(tag => allTags.add(tag)));
        ...
        const tarjetasSlidersHTML = components.sliders.map(slider => `
            <div class="component-card-layout filter-item" data-id="${slider.id}" data-tags="${slider.tags.join(',').toLowerCase()}">
                ...
                <div class="export-actions-panel">
                    <button class="action-copy-btn" data-type="html" data-code="${escapeAttribute(slider.htmlCode)}">
                        <span class="icon">&lt;/&gt;</span> Copiar HTML
                    </button>
                    <button class="action-copy-btn" data-type="css" data-code="${escapeAttribute(slider.cssCode)}">
                        <span class="icon">#</span> Copiar CSS
                    </button>
                    <button class="action-copy-btn" data-type="js" data-code="${escapeAttribute(slider.jsCode)}">
        ...
        return `
            <div class="components-container">
                ...
                <h2>Galería de Sliders</h2>
                <p class="subtitle">Carruseles dinámicos y presentaciones interactivas.</p>
                
                <div class="modern-filters-bar">
                    <div class="search-box">
                        <i class="fa-solid fa-magnifying-glass search-icon"></i>
                        <input type="text" id="component-search" placeholder="Buscar sliders...">
                    </div>
                ...
                <div class="components-list sliders-grid" id="components-list-container">
                ${tarjetasSlidersHTML}
                </div>
            </div>
        `;
    ...
    init: () => {
        ...
        const fuse = new window.Fuse(components.sliders, {
            keys: ['name', 'tags'],
            threshold: 0.4,
            ignoreLocation: true
        });
