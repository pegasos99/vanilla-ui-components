...
export default {
    render: () => {
        ...
        components.themeToggles.forEach(t => t.tags.forEach(tag => allTags.add(tag)));
        ...
        const tarjetasTogglesHTML = components.themeToggles.map(toggle => `
            <div class="component-card-layout filter-item" data-id="${toggle.id}" data-tags="${toggle.tags.join(',').toLowerCase()}">
                ...
                <div class="export-actions-panel">
                    <button class="action-copy-btn" data-type="html" data-code="${escapeAttribute(toggle.htmlCode)}">
                        <span class="icon">&lt;/&gt;</span> Copiar HTML
                    </button>
                    <button class="action-copy-btn" data-type="css" data-code="${escapeAttribute(toggle.cssCode)}">
                        <span class="icon">#</span> Copiar CSS
                    </button>
                    ${toggle.jsCode && toggle.jsCode.trim() !== '' ? `
                    <button class="action-copy-btn" data-type="js" data-code="${escapeAttribute(toggle.jsCode)}">
        ...
        return `
            <div class="components-container">
                ...
                <h2>Theme Toggles</h2>
                <p class="subtitle">Interruptores interactivos para modo oscuro y claro.</p>
                
                <div class="modern-filters-bar">
                    <div class="search-box">
                        <i class="fa-solid fa-magnifying-glass search-icon"></i>
                        <input type="text" id="component-search" placeholder="Buscar toggles...">
                    </div>
    ...
    init: () => {
        ...
        const fuse = new window.Fuse(components.themeToggles, {
            keys: ['name', 'tags'],
            threshold: 0.4,
            ignoreLocation: true
        });
